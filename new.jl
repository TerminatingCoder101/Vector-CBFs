...
Run: julia new.jl                 # static PNG and live-dashboard MP4
Fast static-only run: julia new.jl --no-animate
"""
...
"""
For h=||p-c||²-SAFE_DISTANCE², construct
    Arow*[a,kappa] >= brow
...
geometric_h(state)=minimum(norm(state[1:2]-c)^2-SAFE_DISTANCE^2 for c in OBSTACLES)
...
function cbf_safe_control(state,nominal)
    ...
    for center in OBSTACLES
        ...
        if norm(state[1:2]-center)-OBSTACLE_RADIUS<=CBF_ACTIVATION_DISTANCE
...
function braking_cbf_row(state,center)
    ...
    normal=relative/distance;gap=distance-SAFE_DISTANCE
...
function acbf_control(state,previous_mode)
    ...
    for center in OBSTACLES
        gap=norm(state[1:2]-center)-SAFE_DISTANCE
...
function vcbf_reference(state,latch)
    ...
    for (i,center) in enumerate(OBSTACLES)
        ...
        normal=relative/distance;tangent=perp(normal);gap=distance-SAFE_DISTANCE
        obstacle_vector=center-p
        along=dot(goal_direction,obstacle_vector)
        off=norm(obstacle_vector-along.*goal_direction)
        lane=SAFE_DISTANCE+PRELOAD_LANE
        front=smoothstep(along/0.30)*smoothstep((goal_distance+OBSTACLE_RADIUS-along)/0.30)*
    ...
    for center in OBSTACLES
        ...
        gap=distance-SAFE_DISTANCE;blend=exp(-max(gap,0.0)^2/(2*0.35^2))
...
function energy_cbf_row(state,center)
    ...
    normal=relative/distance;projector=I-normal*normal';gap=distance-SAFE_DISTANCE
...
function maneuver_score(state, direction, first_safe, mode, previous_mode)
    ...
    for k in 1:n_predict
        ...
        clearance = minimum(norm(predicted[1:2]-c)-SAFE_DISTANCE for c in OBSTACLES)
...
function run_controller(controller)
    ...
    clearance = minimum(norm(states[k][1:2]-c)-OBSTACLE_RADIUS-AGENT_RADIUS
                        for k in eachindex(states), c in OBSTACLES)
...
function plot_result(result;output="stationary_obstacle_cbf.png")
    ...
    for (i,c) in enumerate(OBSTACLES)
        poly!(ax,Circle(Point2f(c...),OBSTACLE_RADIUS),color=(:gray,0.65),
              strokecolor=:black,strokewidth=2)
        lines!(ax,Circle(Point2f(c...),SAFE_DISTANCE),color=:gray35,linestyle=:dot)
...
function animate_result(result;output="stationary_obstacle_cbf.mp4")
    ...
    for (i,c) in enumerate(OBSTACLES)
        poly!(ax,Circle(Point2f(c...),OBSTACLE_RADIUS),color=(:gray,0.65),
              strokecolor=:black,strokewidth=2)
        lines!(ax,Circle(Point2f(c...),SAFE_DISTANCE),color=:gray35,
...
function draw_obstacles!(axis)
    for (i,center) in enumerate(OBSTACLES)
        poly!(axis,Circle(Point2f(center...),OBSTACLE_RADIUS),
              color=(:gray,0.65),strokecolor=:black,strokewidth=2)
        lines!(axis,Circle(Point2f(center...),SAFE_DISTANCE),
...
function animate_comparison(results;output="three_controller_comparison.mp4")
    ...
    axislegend(path_axis,position=:rt)
    clock=@lift(@sprintf("comparison time: %.2f s",min(($frame-1)*DT,max_time)))
...
function main(args=ARGS)
    allowed=("--animate","--no-animate","-h","--help")
    any(a->!(a in allowed),args) && error("Usage: julia new.jl [--no-animate]")
    if "-h" in args || "--help" in args
        println("Usage: julia new.jl [--no-animate]\nAnimation is generated by default.")
        return
    end
    results=run_comparison();plot_comparison(results)
    !("--no-animate" in args)&&animate_comparison(results)
end
